...
//for header content
    ...
    headerText += "<div style=\"float:right;\"><form id='theSearch'><input type=\"text\"  title='Search for title or artist by keyword' name='theKeywords' id='theKeywords' value=' Search by title or artist...' onmouseover=\"if (this.value==' Search by title or artist...') this.value = ''\" onmouseout=\"if (this.value=='')this.value=' Search by title or artist...';\" onkeypress=\"if(event.keyCode==13) {newKeyQuery();}\" style=\"background: white url(images/search.png) right no-repeat;padding-right: 17px;\"/></form></div>";
  ...
  function error_handler(response){
      alert("--- Error occurred. ---\n " + error.message);
        ...
        //Image field value manipulation
          ...
          myText += "<img src='http://maps.google.com/maps/api/staticmap?center=";  //need to keep as http - need to open https separately to place in cache 
